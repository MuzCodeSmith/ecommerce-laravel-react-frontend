...
const Show = () => {
    ...
    const onDeleteBrands = async (id) => {
        let res = fetch(`${apiUrl}/brands/${id}`, {
            method: 'DELETE',
            headers: {
                'Content-type': 'application/json',
                'Accept': 'application/json',
    ...
    return (
        <DashboardLayout pagetitle={'Brands'} btnLabel={'Create'} btnLink={'/admin/brands/create'}>
            {
                loader && <Loader />
            }
            {
            ...
            }
            {
                ...
                <table className="table table-hover">
                    ...
                    <tbody>
                        {
                            brands.map(brand => <tr key={brand.id}>
                                <td>{brand.id}</td>
                                <td>{brand.name}</td>
                                <td>
                                    {
                                ...
                                <td>
                                    <Link to={`/admin/brands/edit/${brand.id}`} className='btn text-primary'>
                                    ...
                                    <Link onClick={() => onDeleteBrands(brand.id)} className='btn text-danger ms-2' >
