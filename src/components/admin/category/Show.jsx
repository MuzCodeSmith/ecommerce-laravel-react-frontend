...
const Show = () => {
  ...
  const onDeleteCategory = async (id) => {
    let res = await fetch(`${apiUrl}/categories/${id}`, {
      method: 'DELETE',
      headers: {
        'Content-type': 'application/json',
        'Accept': 'application/json',
  ...
  return (
    <DashboardLayout pagetitle={'Category'} btnLabel={'Create'} btnLink={'/admin/categories/create'} >
      ...
      {
        categories && categories.length > 0 && <table className="table table-hover">
          <thead>
            <tr>
              <th width="50" >ID</th>
              <th>Name</th>
          ...
          <tbody>
            {
              categories.map(category => <tr key={category.id}>
                ...
                <td>
                  <Link to={`/admin/categories/edit/${category.id}`} className='btn text-primary'>
                  ...
                  <Link onClick={() => onDeleteCategory(category.id)} className='btn text-danger ms-2' >
